...
### 配置文件解析[#](https://vitejs.cn/config/#config-file-resolving)

当以命令行方式运行 `vite` 时，Vite 会自动解析 [项目根目录](https://vitejs.cn/guide/#index-html-and-project-root) 下名为 `vite.config.js` 的文件。
...
### 配置智能提示[#](https://vitejs.cn/config/#config-intellisense)
...
### 情景配置[#](https://vitejs.cn/config/#conditional-config)

如果配置文件需要基于（`dev`/`serve` 或 `build`）命令或者不同的 [模式](https://vitejs.cn/guide/env-and-mode.html) 来决定选项，则可以选择导出这样一个函数：
...
### 异步配置[#](https://vitejs.cn/config/#async-config)
...
## 共享配置[#](https://vitejs.cn/config/#shared-options)

### root[#](https://vitejs.cn/config/#root)
...
- **默认：** `process.cwd()`
    ...
    更多细节请见 [项目根目录](https://vitejs.cn/guide/#index-html-and-project-root)。
...
- **默认：** `/`
    ...
    更多信息详见 [公共基础路径](https://vitejs.cn/guide/build.html#public-base-path)。
    

### mode[#](https://vitejs.cn/config/#mode)
...
- **默认：** `'development'`（开发模式），`'production'`（生产模式）
    ...
    查看 [环境变量与模式](https://vitejs.cn/guide/env-and-mode.html) 章节获取更多细节。
    

### define[#](https://vitejs.cn/config/#define)
...
### plugins[#](https://vitejs.cn/config/#plugins)

- **类型：** `(Plugin | Plugin[])[]`
    
    需要用到的插件数组。Falsy 虚值的插件将被忽略，插件数组将被扁平化（flatten）。查看 [插件 API](https://vitejs.cn/guide/api-plugin.html) 获取 Vite 插件的更多细节。
    

### publicDir[#](https://vitejs.cn/config/#publicdir)
...
- **默认：** `"public"`
    ...
    欲了解更多，请参阅 [`public` 目录](https://vitejs.cn/guide/assets.html#the-public-directory)。
    

### cacheDir[#](https://vitejs.cn/config/#cachedir)
...
- **类型：**`Record<string, string> | Array<{ find: string | RegExp, replacement: string }>`
    ...
    更高级的自定义解析方法可以通过 [插件](https://vitejs.cn/guide/api-plugin.html) 实现。
    

### resolve.dedupe[#](https://vitejs.cn/config/#resolve-dedupe)
...
### resolve.conditions[#](https://vitejs.cn/config/#resolve-conditions)
...
### resolve.mainFields[#](https://vitejs.cn/config/#resolve-mainfields)
...
### resolve.extensions[#](https://vitejs.cn/config/#resolve-extensions)
...
### resolve.preserveSymlinks[#](https://vitejs.cn/config/#resolve-preservesymlinks)
...
### json.namedExports[#](https://vitejs.cn/config/#json-namedexports)
...
### json.stringify[#](https://vitejs.cn/config/#json-stringify)
...
### esbuild[#](https://vitejs.cn/config/#esbuild)
...
### assetsInclude[#](https://vitejs.cn/config/#assetsinclude)

- **类型：** `string | RegExp | (string | RegExp)[]`
    
- **相关内容：** [静态资源处理](https://vitejs.cn/guide/assets.html)
...
### logLevel[#](https://vitejs.cn/config/#loglevel)
...
### clearScreen[#](https://vitejs.cn/config/#clearscreen)
...
### envDir[#](https://vitejs.cn/config/#envdir)
...
- **默认：** `root`
    ...
    关于环境文件的更多信息，请参见 [这里](https://vitejs.cn/guide/env-and-mode.html#env-files)。
    

### envPrefix[#](https://vitejs.cn/config/#envprefix)
...
## 开发服务器选项[#](https://vitejs.cn/config/#server-options)

### server.host[#](https://vitejs.cn/config/#server-host)
...
### server.port[#](https://vitejs.cn/config/#server-port)
...
### server.strictPort[#](https://vitejs.cn/config/#server-strictport)
...
### server.https[#](https://vitejs.cn/config/#server-https)
...
### server.open[#](https://vitejs.cn/config/#server-open)
...
### server.proxy[#](https://vitejs.cn/config/#server-proxy)
...
### server.cors[#](https://vitejs.cn/config/#server-cors)
...
### server.force[#](https://vitejs.cn/config/#server-force)

- **类型：** `boolean`
    
- **相关内容：** [依赖预构建](https://vitejs.cn/guide/dep-pre-bundling.html)
    
    设置为 `true` 强制使依赖预构建。
    

### server.hmr[#](https://vitejs.cn/config/#server-hmr)
...
### server.watch[#](https://vitejs.cn/config/#server-watch)
...
### server.middlewareMode[#](https://vitejs.cn/config/#server-middlewaremode)
...
- **相关：** [SSR - 设置开发服务器](https://vitejs.cn/guide/ssr.html#setting-up-the-dev-server)
...
### server.fs.strict[#](https://vitejs.cn/config/#server-fs-strict)
...
### server.fs.allow[#](https://vitejs.cn/config/#server-fs-allow)

- **类型：** `string[]`
    
    限制哪些文件可以通过 `/@fs/` 路径提供服务。当 `server.fs.strict` 设置为 true 时，访问这个目录列表外的文件将会返回 403 结果。
    
    Vite 将会搜索此根目录下潜在工作空间并作默认使用。一个有效的工作空间应符合以下几个条件，否则会默认以 [项目 root 目录](https://vitejs.cn/guide/#index-html-and-project-root) 作备选方案。
    
    - 在 `package.json` 中包含 `workspaces` 字段
    - 包含以下几种文件之一
        - `pnpm-workspace.yaml`
    
    接受一个路径作为自定义工作区的 root 目录。可以是绝对路径或是相对于 [项目 root 目录](https://vitejs.cn/guide/#index-html-and-project-root) 的相对路径。示例如下：
...
### server.fs.deny[#](https://vitejs.cn/config/#server-fs-deny)
...
### server.origin[#](https://vitejs.cn/config/#server-origin)
...
## 构建选项[#](https://vitejs.cn/config/#build-options)
...
- **相关内容：:** [浏览器兼容性](https://vitejs.cn/guide/build.html#browser-compatibility)
...
### build.polyfillModulePreload[#](https://vitejs.cn/config/#build-polyfillmodulepreload)
...
- **默认值：** `true`
    ...
    注意：此 polyfill **不适用于** [Library 模式](https://vitejs.cn/guide/build.html#library-mode)。如果你需要支持不支持动态引入的浏览器，你应该避免在你的库中使用此选项。
    

### build.outDir[#](https://vitejs.cn/config/#build-outdir)
...
### build.assetsDir[#](https://vitejs.cn/config/#build-assetsdir)
...
### build.assetsInlineLimit[#](https://vitejs.cn/config/#build-assetsinlinelimit)
...
### build.cssCodeSplit[#](https://vitejs.cn/config/#build-csscodesplit)
...
### build.sourcemap[#](https://vitejs.cn/config/#build-sourcemap)
...
### build.rollupOptions[#](https://vitejs.cn/config/#build-rollupoptions)
...
### build.commonjsOptions[#](https://vitejs.cn/config/#build-commonjsoptions)
...
### build.dynamicImportVarsOptions[#](https://vitejs.cn/config/#build-dynamicimportvarsoptions)
...
### build.lib[#](https://vitejs.cn/config/#build-lib)

- **类型：** `{ entry: string, name?: string, formats?: ('es' | 'cjs' | 'umd' | 'iife')[], fileName?: string | ((format: ModuleFormat) => string) }`
    
- **相关内容：** [库模式](https://vitejs.cn/guide/build.html#library-mode)
...
### build.manifest[#](https://vitejs.cn/config/#build-manifest)

- **类型：** `boolean`
    
- **默认：** `false`
    
- **相关内容：** [后端集成](https://vitejs.cn/guide/backend-integration.html)
    
    当设置为 `true`，构建后将会生成 `manifest.json` 文件，包含了没有被 hash 的资源文件名和 hash 后版本的映射。可以为一些服务器框架渲染时提供正确的资源引入链接。
    

### build.ssrManifest[#](https://vitejs.cn/config/#build-ssrmanifest)

- **类型：** `boolean`
    
- **默认值：** `false`
    
- **相关链接：** [Server-Side Rendering](https://vitejs.cn/guide/ssr.html)
    
    当设置为 `true` 时，构建也将生成 SSR 的 manifest 文件，以确定生产中的样式链接与资产预加载指令。
    

### build.ssr[#](https://vitejs.cn/config/#build-ssr)
...
### build.minify[#](https://vitejs.cn/config/#build-minify)
...
### build.write[#](https://vitejs.cn/config/#build-write)
...
### build.emptyOutDir[#](https://vitejs.cn/config/#build-emptyoutdir)
...
### build.brotliSize[#](https://vitejs.cn/config/#build-brotlisize)
...
### build.chunkSizeWarningLimit[#](https://vitejs.cn/config/#build-chunksizewarninglimit)
...
### build.watch[#](https://vitejs.cn/config/#build-watch)
...
## 预览选项[#](https://vitejs.cn/config/#preview-options)

### preview.host[#](https://vitejs.cn/config/#preview-host)

- **类型：** `string | boolean`
    
- **默认：** [`server.host`](https://vitejs.cn/config/#server_host)
...
### preview.port[#](https://vitejs.cn/config/#preview-port)
...
### preview.strictPort[#](https://vitejs.cn/config/#preview-strictport)

- **类型：** `boolean`
    
- **默认：** [`server.strictPort`](https://vitejs.cn/config/#server_strictport)
    
    设置为 `true` 时，如果端口已被使用，则直接退出，而不会再进行后续端口的尝试。
    

### preview.https[#](https://vitejs.cn/config/#preview-https)

- **类型：** `boolean | https.ServerOptions`
    
- **默认：** [`server.https`](https://vitejs.cn/config/#server_https)
...
### preview.open[#](https://vitejs.cn/config/#preview-open)

- **类型：** `boolean | string`
    
- **默认：** [`server.open`](https://vitejs.cn/config/#server_open)
...
### preview.proxy[#](https://vitejs.cn/config/#preview-proxy)

- **类型：** `Record<string, string | ProxyOptions>`
    
- **默认：** [`server.proxy`](https://vitejs.cn/config/#server_proxy)
...
### preview.cors[#](https://vitejs.cn/config/#preview-cors)

- **类型：** `boolean | CorsOptions`
    
- **默认：** [`server.cors`](https://vitejs.cn/config/#server_proxy)
    
    为开发服务器配置 CORS。此功能默认启用，支持任何来源。可传递一个 [options 对象](https://github.com/expressjs/cors) 来进行配置，或者传递 `false` 来禁用此行为。
    

## 依赖优化选项[#](https://vitejs.cn/config/#dep-optimization-options)

- **相关内容：** [依赖预构建](https://vitejs.cn/guide/dep-pre-bundling.html)

### optimizeDeps.entries[#](https://vitejs.cn/config/#optimizedeps-entries)
...
### optimizeDeps.exclude[#](https://vitejs.cn/config/#optimizedeps-exclude)
...
### optimizeDeps.include[#](https://vitejs.cn/config/#optimizedeps-include)
...
### optimizeDeps.keepNames[#](https://vitejs.cn/config/#optimizedeps-keepnames)
...
## SSR 选项[#](https://vitejs.cn/config/#ssr-options)

实验性

SSR 选项可能会在未来版本中进行调整。

- **相关：** [SSR 外部化](https://vitejs.cn/guide/ssr.html#ssr-externals)

### ssr.external[#](https://vitejs.cn/config/#ssr-external)
...
### ssr.noExternal[#](https://vitejs.cn/config/#ssr-noexternal)
...
### ssr.target[#](https://vitejs.cn/config/#ssr-target)
